...
export default class Get extends React.Component {
  ...
  componentDidMount() {
    axios.get(`http://localhost:8080/loan`).then((res) => {
  ...
  deleteRow(loanId, e){
    axios.delete(`http://localhost:8080/loan/${loanId}`)
      .then(res => {
        console.log(res);
            ...
            {this.state.posts.map((post,i) => (
              <tr key={i}>
                ...
                <td>{post.loantype}</td>
                 ...
                 <td><button onClick={(e) => this.deleteRow(post.loanId, e)} class="fa-solid fa-trash">Delete</button></td>
